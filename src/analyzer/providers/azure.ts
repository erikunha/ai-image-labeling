...
export function createAzureAsyncBatchClient(config: Config): AsyncBatchClient {
  // Batch API requires 2024-07-01 or later; the sync client uses 2024-02-01.
  const azure = new AzureOpenAI({
    endpoint: config.azureEndpoint ?? '',
    apiKey: config.azureApiKey ?? '',
  ...
  return {
    async submitBatch(requests: readonly AsyncBatchRequest[]) {
      ...
      for (const req of requests) {
        ...
        for (const img of req.images) {
          ...
          content.push({
            type: 'image_url',
            image_url: { url: `data:image/jpeg;base64,${img.base64}`, detail: req.opts.detail ?? 'low' },
          });
        }
        const messages: OpenAI.Chat.ChatCompletionMessageParam[] = [];
      ...
      const file = await azure.files.create({
        file: await toFile(Buffer.from(jsonl, 'utf-8'), 'batch.jsonl', { type: 'application/json' }),
        purpose: 'batch',
      });

      const batch = await azure.batches.create({
        input_file_id: file.id,
        endpoint: '/v1/chat/completions',
        completion_window: '24h',
      });

      logger.verbose(`  Azure async batch created: ${batch.id} (file: ${file.id})`);
      return { jobId: batch.id, customIds };
    },

    async checkStatus(jobId: string) {
      const batch = await azure.batches.retrieve(jobId);
    ...
    async retrieveResults(jobId: string, customIds: readonly string[]) {
      ...
      if (!batch.output_file_id) {
        throw new Error(`Azure batch ${jobId} has no output file (status: ${batch.status})`);
      }

      const fileContent = await azure.files.content(batch.output_file_id);
      ...
      for (const line of text.split('\n').filter((l) => l.trim())) {
        const entry = JSON.parse(line) as {
          custom_id: string;
          response: { status_code: number; body: { choices: Array<{ message: { content: string | null } }> } };
          error?: unknown;
        };
