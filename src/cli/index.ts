...
import { runBatch, runReorder, runReport, runSearch, runServe, runSingle, runSuggestCategories, runWatch } from '../index.js';
...
program
  ...
  .option('--provider <name>', 'LLM provider: openai | anthropic | google | bedrock | vertex', 'openai')
  ...
  .option('--bedrock-access-key-id <key>', 'AWS access key ID for Bedrock (overrides AWS_ACCESS_KEY_ID)')
  .option('--bedrock-secret-access-key <key>', 'AWS secret access key for Bedrock (overrides AWS_SECRET_ACCESS_KEY)')
  .option('--vertex-project <id>', 'Google Cloud project ID for Vertex AI (overrides GOOGLE_CLOUD_PROJECT)')
  .option('--vertex-location <location>', 'Google Cloud location for Vertex AI (default: us-central1)')
  ...
  .option('--output-format <fmt>', 'Output format: pretty | json | none | csv | xlsx | sqlite', 'pretty')
  ...
  .option('--link-window <days>', 'Time window in days for grouping images in the linking pass', '7')
  .option(
    '--self-critique',
...
program
  ...
  .option('-i, --input <dir>', 'Input directory with source images', './input')
  .option('--out <file>', 'Output path for the suggested categories.json', './categories-suggested.json')
...
program
  ...
  .option(
    '--provider <name>',
    'LLM provider (required for --query semantic search)',
    'openai',
  )
