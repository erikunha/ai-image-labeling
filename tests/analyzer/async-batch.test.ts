...
describe('submitAsyncBatch', () => {
  it('calls submitBatch with one group per batchSize', async () => {
    ...
    const submitBatch = vi.fn().mockResolvedValue({ jobId: 'job-1', customIds: ['batch-0', 'batch-1'] });
...
describe('resumeAsyncBatch', () => {
  it('polls until complete then returns parsed results in order', async () => {
    ...
    const responseText = JSON.stringify({
      images: [
        { index: 1, category: 'kitchen', shortDescription: 'clean kitchen', elements: [], confidence: 0.95, extractedText: '' },
        { index: 2, category: 'bathroom', shortDescription: 'white tiles', elements: [], confidence: 0.88, extractedText: '' },
      ],
    });
    ...
    const asyncClient: AsyncBatchClient = {
      ...
      retrieveResults: vi.fn().mockResolvedValue([
        { customId: 'batch-0', text: responseText, status: 'success' },
      ]),
    };
  ...
  it('pads with unknown when a batch group fails', async () => {
    ...
    const asyncClient: AsyncBatchClient = {
      ...
      retrieveResults: vi.fn().mockResolvedValue([
        { customId: 'batch-0', text: '{}', status: 'failed' },
      ]),
    };
      ...
      retrieveResults: vi.fn().mockResolvedValue([
        {
          ...
          text: JSON.stringify({
            images: [{
              index: 1, category: 'Living Room', shortDescription: 'test', elements: [],
              confidence: 0.9, extractedText: '',
            }],
          }),
          status: 'success',
        },
